package com.mehedi.service.impl;
...
import com.mehedi.beans.DemandBean;
import com.mehedi.beans.ProductBean;
import com.mehedi.service.ProductService;
import com.mehedi.utility.DBUtil;
import com.mehedi.utility.IDUtil;
import com.mehedi.utility.MailMessage;
...
public class ProductServiceImpl implements ProductService {
	...
	public ProductBean getProductById(String prodId) {
	    ProductBean product = null;

	    // Obtain a connection to the database
	    Connection con = DBUtil.provideConnection();
	    
	    // Prepare the SQL statement and result set
	    PreparedStatement ps = null;
	    ResultSet rs = null;

	    try {
	        // Prepare the SQL query to fetch product details by product ID
	        ps = con.prepareStatement("SELECT * FROM product WHERE pid = ?");
	        ps.setString(1, prodId);
	        ...
	        // If a product is found, create a ProductBean object and populate it
	        if (rs.next()) {
	            product = new ProductBean();
	            product.setProdId(rs.getString(1));          // Assuming column 1 is PID
	            product.setProdName(rs.getString(2));        // Column 2 is product name
	            product.setProdType(rs.getString(3));        // Column 3 is product type
	            product.setProdInfo(rs.getString(4));        // Column 4 is product info
	            product.setProdPrice(rs.getDouble(5));       // Column 5 is product price
	            product.setProdQuantity(rs.getInt(6));       // Column 6 is product quantity
	            product.setProdImage(rs.getAsciiStream(7));   // Column 7 is product image
	        }
	    ...
	    // Return the populated ProductBean or null if not found
	    return product;
	}
